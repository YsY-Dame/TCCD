...
class TrainLoop:
    ...
    def run_loop(self, w=None):
        while (
            ...
            print('######',self.lr_anneal_steps, self.step, self.resume_step)
...
class KDTrainLoop(TrainLoop):
    ...
    def forward_backward(self, x_t_1, x_t, t, model_kwargs, out):
        ...
        for i in range(0, x_t.shape[0], self.microbatch):
            ...
            compute_losses_self_super = functools.partial(
                self.diffusion.training_losses_kd,
                self.ddp_model,
                self.teacher_model,
                micro_x_t_1,
            ...
            loss = (losses["loss"] * weights).mean()
            log_loss_dict(
                self.diffusion, t, losses, self.step+self.resume_step
            )
            self.mp_trainer.backward(loss)
...
def log_loss_dict(diffusion, ts, losses, promote='', q=4):
    logger.logkv('step', str(promote))
