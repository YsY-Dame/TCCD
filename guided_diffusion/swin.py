...
class PatchMerging(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.LayerNorm
    """
    def __init__(self, dim, out_channels, input_resolution, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
...
class SwinT(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads, input_resolution, window_size,
        ...
        for i in range(2):
            self.blocks.append(
                SwinTransformerBlock(
                    dim=in_channels, num_heads=num_heads, window_size=window_size,
                    input_resolution=input_resolution,
                    shift_size=(0, 0) if (i % 2 == 0) else (window_size[0] // 2, window_size[1] // 2),
                    mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=dropout, attn_drop=attention_dropout, drop_path=droppath,
                    act_layer=nn.GELU, norm_layer=nn.LayerNorm
                )
            )

        self.cnn = nn.Conv2d(in_channels, out_channels, 1, 1)

        if downsample:
            self.downsample = PatchMerging(dim=in_channels, out_channels=out_channels, input_resolution=input_resolution)
        else:
